...
def mon_premier_tweet_ruby (client)
...
def select_random_journalists(arr_journalists, number, client)
return arr_journalists.sample(number)

end

def send_tweet_to_random_journalists(arr_journalists, number, client)
  bonjour_monde = "\#bonjour_monde"
  good_work = "You have done an amazing job!"
  thp = "@the_hacking_pro"
  select_random_journalists(arr_journalists, number, client).each do |journalist|
    client.update(journalist + " " + bonjour_monde + " " + good_work + " " + thp)
  end
end

send_tweet_to_random_journalists(journalists, 5, client)